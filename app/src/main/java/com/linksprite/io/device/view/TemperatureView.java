package com.linksprite.io.device.view;
...
public class TemperatureView extends View {
    ...
    private int unit = 1;
    private int halfCount = 20;
    private float currentTemp;
    private String normal = "正常";
    private String warn = "预警";
    private String danger = "警告";
    private String text = "当前温度";
...
// 绘制文字
    ...
    private void drawPanelText(Canvas canvas) {
        ...
        String temp = currentTemp + " " + unitText;
        panelTextPaint.setTextSize(sp2px(15));
...
// 画右半部分的刻度
        int half = mTikeCount / 2;
        for (int i = 0; i <= half; i++) {
// 5的倍数就画长刻度，并标上刻度数值
            if (i % 5 == 0) {
                scale = halfCount + (i * unit) + "";
                panelTextPaint.setTextSize(sp2px(15));
...
// 画布回正
        canvas.rotate(-mAngle * half - 6, 0, 0);
//// 画左半部分的刻度
        for (int i = 0; i <= half; i++) {
            if (i % 5 == 0) {
                scale = halfCount - (i * unit) + "";
